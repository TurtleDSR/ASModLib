...
class UPirateOctopusSecondSequenceCapability : UHazeCapability
{
	...
	TArray<APirateOctopusSecondSequenceSlamArm> PendingArmsActivation;
    ...
    void TickActive(float DeltaTime)
    {
		...
		{
			// CROSSGEN FIX:
			//   Keep the old event for crossgen compatibility purposes
			if (Game::IsConsoleBuild())
			{
				if (!Octopus.WheelBoat.bIsDying
			...
			{
				bAwatingFullSyncPoint = true;
				Sync::FullSyncPoint(Octopus, n"InitalizeThirdAttackSequenceIfNotDead");
			}
		}
	}
	...
	{
		if(PendingArmsActivation.Num() == 0)
			return;
		...
		auto BombDuck = PendingArmsActivation[0];
		...
		{
			BombDuck.CannonBallDamageableComponent.EnableDamageTaking();
		}
		// END CROSSGEN FIX
		PendingArmsActivation.RemoveAt(0);
	...
	void CreateBombDuck(int PositionIndex)
	{
		...
		LastPointIndex = PositionIndex;
		PendingArmsActivation.Add(BombDuck);
