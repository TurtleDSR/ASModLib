...
const FConsoleVariable CVar_AlwaysShowPromo("Haze.AlwaysShowPromo", 0);
...
class UMainMenuWidget : UMainMenuStateWidget
{
	...
	UPROPERTY(Meta = (BindWidget))
	UWidget PromoContainer;
	UPROPERTY(Meta = (BindWidget))
	UMenuPromptOrButton EASignInButton;

	UPROPERTY(Meta = (BindWidget))
	UMenuPromptOrButton PromoButton;
	UPROPERTY(Meta = (BindWidget))
	UWidget PromoLogo;
	UPROPERTY(Meta = (BindWidget))
	UWidget PromoLogoAndQRCode;
	...
	void Construct()
	{
		...
		PromoButton.OnPressed.AddUFunction(this, n"OnPromoClicked");
	}
	...
	void Show(bool bSnap) override
	{
		...
		if (Online::ShouldShowPromo() || CVar_AlwaysShowPromo.GetInt() != 0)
		{
			if ((Online::IsPromoPreRelease() && CVar_AlwaysShowPromo.GetInt() == 0) || CVar_AlwaysShowPromo.GetInt() == 2 || CVar_AlwaysShowPromo.GetInt() == 3)
			{
				if (Game::PlatformName == "PS5" || CVar_AlwaysShowPromo.GetInt() == 3)
				{
					PromoLogoAndQRCode.Visibility = ESlateVisibility::SelfHitTestInvisible;
					PromoLogo.Visibility = ESlateVisibility::Collapsed;
				}
				else
				{
					PromoLogoAndQRCode.Visibility = ESlateVisibility::Collapsed;
					PromoLogo.Visibility = ESlateVisibility::SelfHitTestInvisible;
				}
				PromoButton.Text = NSLOCTEXT("MainMenu", "PromoTextPreRelease", "Discover our upcoming game");
			}
			else
			{
				PromoLogoAndQRCode.Visibility = ESlateVisibility::Collapsed;
				PromoLogo.Visibility = ESlateVisibility::SelfHitTestInvisible;
				PromoButton.Text = NSLOCTEXT("MainMenu", "PromoText", "Discover our latest game");
			}

			PromoContainer.Visibility = ESlateVisibility::SelfHitTestInvisible;
		}
		else
		{
			PromoContainer.Visibility = ESlateVisibility::Collapsed;
		}
	}
	...
	void MainMenu_HostOnlineGame()
	{
		...
		if (OnlinePlatformName == "Steam" || OnlinePlatformName == "Editor")
			Online::SetGameServerMode(EHazeGameServerMode::DelayHosting);
		else
